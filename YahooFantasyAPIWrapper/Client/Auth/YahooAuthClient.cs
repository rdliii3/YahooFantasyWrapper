    ...
    public class YahooAuthClient : IYahooAuthClient
    {
        ...
        protected async Task<UserInfo> GetUserInfo()
        {
            var endPoint = new EndPoint
            {
                ...
                Resource = AuthApiEndPoints.UserInfoServiceEndpoint.Resource
            };
        ...
        private async Task QueryAccessToken(NameValueCollection parameters)
        {
            ...
            var authHeaderString = $"{Configuration.Value.ClientId}:{Configuration.Value.ClientSecret}";
            var authHeaderStringBase64Encoded = System.Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(authHeaderString));
            request.Headers.Add("Authorization", $"Basic {authHeaderStringBase64Encoded}");
